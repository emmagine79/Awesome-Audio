...
struct ProcessingCoordinatorTests {
    ...
    @Test func processPreservesOutputFrameCountAfterLatencyCompensation() async throws {
        let coordinator = ProcessingCoordinator()
        ...
        let decoded = try AudioTestHelpers.readMonoFloatSamples(from: result.tempOutputURL)
        #expect(decoded.count == input.count, "Processed output should preserve input frame count")
    }
}
